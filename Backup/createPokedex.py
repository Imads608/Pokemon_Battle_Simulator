...
def getPokedex(fileName, typesMap, pokemonImageMap):
    ...
    for line in allLines:
        ...
        if ("[" in line and "]" in line):
            ...
            if (newPokemonFound > 1):
                ...
                pokemonEntry = Pokemon_Details(str(pokedexNumber-1), pokemonFullName, pokemonCodeName, pokemonTypes, matchStats, baseExp, happinessValue, matchAbilities, hiddenAbility, matchEggMoves, movesList, weaknesses, resistances, immunities, pokemonImageFile)
    ...
    pokemonEntry = Pokemon_Details(pokedexNumber, pokemonFullName, pokemonCodeName, pokemonTypes, matchStats, baseExp,
    ...
    pokedex.update({pokemonCodeName:pokemonEntry})
    getPokemonTMs("Pokemon Essentials v16 2015-12-07/PBS/tm.txt", pokedex)
    return pokedex
...
def getPokemonMatchups(pokemonTypes, typesMap):
    ...
    for weakness, damage_ratio in accumulatedWeakness:
        if ((weakness, "x0.5") in accumulatedResistances):
            ...
            if (damage_ratio == "x2"):
                pokemonWeakness.remove((weakness, damage_ratio))
                indexResistance = pokemonResistances.index((weakness, "x0.25"))
                pokemonResistance[indexResistance] = (weakness, "x0.5")
            else:
...
def getFunctionCodes(fileName):
    ...
    with open(fileName, 'r') as functionCodes:
        allLines = functionCodes.readlines()

    allLines = allLines[0].split("\r")
    xlsWorkbook = xlrd.open_workbook("Function_Codes.xlsx")
    ...
    for row in range(numRows):
        newCodeFlag = 0
        descriptionFlag = 0
        codeSetDescriptionFlag = 0

        splitCode = str(worksheet.cell(row, 0).value).split(".")
        value = splitCode[0]
        matchHex = re.search(r'[a-zA-Z]', str(worksheet.cell(row, 0).value))
        if (matchHex == None):
           matchNum = 10
        else:
            matchNum = 16
        ...
        try:
            int(value, base=matchNum)
            newCodeFlag = 1
            addMovesFlag = 0
            #codeEffect = ""
        except:
            if (value != "" and value != "Function code"):
                fullMapUpdateFlag = 0
                if (len(mapFuncCode) != 0):
                    fullMapUpdateFlag = 1
                    prevRangeEffect = codeRangeEffect
                codeRangeEffect = worksheet.cell(row, 0).value

        if (fullMapUpdateFlag == 1):
            fullMap.update({prevRangeEffect: mapFuncCode})
            mapFuncCode = {}
            codeEffect = ""
            fullMapUpdateFlag = 0

        if (newCodeFlag == 1):
            if (functionCode != "" and len(moveFullNameList) != 0):
                mapFuncCode.update({functionCode:(codeEffect, moveFullNameList)})
                codeEffect = ""

            functionCode = value #worksheet.cell(row, 0).value
            moveFullNameList = []
            newCodeFlag = 0
            addMovesFlag = 1
            codeEffect = codeEffect + worksheet.cell(row, 1).value

        if (addMovesFlag == 1):
            moveFullNameList.append(worksheet.cell(row, 2).value)
...
if __name__ == "__main__":
    mapAbilities = allAbilities("Pokemon Essentials v16 2015-12-07/PBS/abilities.txt")
    moveFlags = getMoveFlags()
    mapMoves = allMoves("Pokemon Essentials v16 2015-12-07/PBS/moves.txt")
    targetFlags = getTargetFlags()
    pokemonImageMap = getPokemonImages("img/*")
    typesMap = getAllTypes("Pokemon Essentials v16 2015-12-07/PBS/types.txt")
    pokedex = getPokedex("Pokemon Essentials v16 2015-12-07/PBS/pokemon.txt", typesMap, pokemonImageMap)
    itemsMap = allItems("Pokemon Essentials v16 2015-12-07/PBS/items.txt")
    pocketMap = definePocket()
    usabilityInMap = defineUsabilityInBattle()
    usabilityOutMap = defineUsabilityOutBattle()
    functionCodesMap = getFunctionCodes("Function_Codes.csv")
