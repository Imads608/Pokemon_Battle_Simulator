...
class battleConsumer(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self, parent = None):
        ...
        self.player1Team = []
        self.player2Team = []
        self.player1Properties = [self.listPlayer1_team, self.player1Team, self.viewPokemon1, self.listPokemon1_moves, self.txtPokemon1_Level, self.txtPokemon1_Ability,
                                  self.txtPokemon1_HP, self.txtPokemon1_Nature]
        self.player2Properties = [self.listPlayer2_team, self.player2Team, self.viewPokemon2, self.listPokemon2_moves, self.txtPokemon2_Level,
                                  self.txtPokemon2_Ability, self.txtPokemon2_HP, self.txtPokemon2_Nature]
        ...
        self.listPlayer1_team.doubleClicked.connect(lambda:self.seeStats(self.player1Properties))
        self.listPlayer2_team.doubleClicked.connect(lambda:self.seeStats(self.player2Properties))
...
###########################     TAB 1 DEFENITIONS  ##########################################
    ...
    def seeStats(self, playerDetails): #listPlayerTeam, playerTeam, viewPokemon):
        listPlayerTeam = playerDetails[0]
        playerTeam = playerDetails[1]
        viewPokemon = playerDetails[2]
        currItem = listPlayerTeam.currentItem()

        for pokemon in playerTeam:
            if (currItem.text() == pokemon.name):
                self.seePokemon(pokemon, viewPokemon)
                

    def seePokemon(self, pokemon, viewPokemon):
        self.chosenPokemon = pokemon.entry
        self.displayPokemon(viewPokemon)
